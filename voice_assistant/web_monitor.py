...
def _active_env_file_from_snapshot(snapshot: dict[str, Any]) -> Path:
    ...
    return env_dir / (".env.offline" if mode == "offline" else ".env.online")
...
class WebMonitor(_BaseWebMonitor):
    ...
    def _save_voice_engine(self, engine: str, *, realtime_model: str = "", realtime_voice: str = "") -> dict[str, Any]:
        ...
        if normalized not in allowed:
            raise ValueError(f"voice_engine must be one of: {', '.join(sorted(allowed))}")
    ...
    def _save_voice_output_gains(self, cloud_gain: Any, local_gain: Any) -> dict[str, Any]:
        ...
        updates = {"CLOUD_TTS_OUTPUT_GAIN": f"{cloud:.2f}", "LOCAL_TTS_OUTPUT_GAIN": f"{local:.2f}"}
        written: list[str] = []
        ...
        return {"ok": True, "cloud_gain": cloud, "local_gain": local, "profiles": written, "restart_required": True, "message": "Speech output gain saved. Restart LiveStageAssistant to apply it."}
            ...
            def server_factory(server_address, handler_class):
                class RealtimePolicyHandler(handler_class):
                    ...
                    def do_POST(self) -> None:
                        parsed = _base.urlparse(self.path)
                        routes = {"/api/mcp-realtime-policy", "/api/mcp-server", "/api/voice-engine", "/api/voice-output-gains", "/api/runtime-restart"}
                        ...
                        if parsed.path == "/api/voice-engine":
                            self._handle_voice_engine_save(); return
                        if parsed.path == "/api/voice-output-gains":
                            self._handle_voice_output_gains_save(); return
                    ...
                    def _handle_voice_engine_save(self) -> None:
                        ...
                        try:
                            result = monitor._save_voice_engine(str(payload.get("voice_engine") or ""), realtime_model=str(payload.get("realtime_model") or ""), realtime_voice=str(payload.get("realtime_voice") or ""))
                        except ValueError as error:
                            self._send_json_error(400, {"ok": False, "error": {"message": str(error)}}); return
                        except Exception as error:
                            self._send_json_error(500, {"ok": False, "error": {"message": f"Could not save voice engine: {error}"}}); return
                    ...
                    def _handle_voice_output_gains_save(self) -> None:
                        ...
                        try:
                            ...
                            self._send_json_error(500, {"ok": False, "error": {"message": f"Could not save voice output gains: {error}"}}); return
                        self._send_json(result)
