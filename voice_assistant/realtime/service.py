...
This is production-facing runtime code, not a diagnostic runner. It loads the
active env profile, canonical MCP inventory and backend audio selectors, then
runs one OpenAI Realtime session with any configured provider-native MCPs plus
local STDIO MCP bridge tools in the same session.
"""
...
async def native_server(server: CanonicalMCPServerConfig) -> RealtimeMCPServer:
...
async def execute_bridge_call(engine, bridge: RealtimeMCPBridge, event, completed_calls: set[str]) -> None:
    ...
    print(
        "Realtime bridge call " + json.dumps(
            {"server": target.server if target else None, "tool": target.tool if target else name, "arguments": arguments},
            ensure_ascii=False,
            separators=(",", ":"),
        ),
        flush=True,
    ...
    while not stop_event.is_set():
        ...
        if event.type == "speech_started":
            ...
            metrics = {
                ...
                "speech_end_to_first_audio_ms": round((first_audio_received[response_id]-speech_end)*1000, 1) if speech_end is not None and response_id in first_audio_received else None,
                "speech_end_to_first_playback_ms": round((first_played[response_id]-speech_end)*1000, 1) if speech_end is not None and response_id in first_played else None,
                "response_start_to_done_ms": round((now-response_started[response_id])*1000, 1) if response_id in response_started else None,
...
async def run(args) -> int:
    ...
    config_value = str(os.getenv("MCP_CONFIG") or "mcp_servers.json").strip()
    ...
    for server in inventory.values():
        ...
        if transport == "auto":
            transport = "native" if server.native.url else "stdio"
            print(f"Realtime MCP auto selection: {server.name} -> {transport}", flush=True)
        if transport == "native":
    ...
    try:
        ...
        engine = OpenAIRealtimeEngine(
            RealtimeEngineConfig(
                ...
                model=model,
                voice=voice,
                instructions=DEFAULT_BASE_PROMPT,
                server_vad=True,
        ...
        print("Speak naturally; provider VAD handles turns and interruption.", flush=True)
