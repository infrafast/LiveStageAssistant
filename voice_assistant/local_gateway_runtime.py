...
def _brief_gateway_error(server: str, error: Exception, *, locale: str = "fr") -> str:
    """Return a localized user-facing error while logs retain the full exception."""
    return localized_error_text(locale, domain=server, error=error)
        ...
        *,
        ...
        if not claims:
            if failures:
                ...
                return _brief_gateway_error(server, error, locale=self.locale)
            return "Commande non reconnue."
        if len(claims) > 1:
