...
DEFAULT_ASSISTANT_SYSTEM_PROMPT = (
    ...
    "If a tool is needed, emit no spoken or textual assistant content before the tool result. Do not announce an intention before a tool call, including variants of I will check, let me check, I am going to, je vais vérifier, je regarde, je vais faire, d'accord je vais. "
    "Do not produce filler, acknowledgement, narration, or progress text before a tool call, including variants of d'accord, ok, très bien, un instant, je vérifie. "
...
class VoiceAssistant:
    ...
    def __init__(
        ...
        Args:
            ...
            command_ack_sound_file: WAV cue played when a command response is ready
        ...
        self.listening_sound_file = (listening_sound_file or "").strip()
        self.listening_sound_path = (
            self._resolve_asset_path(self.listening_sound_file)
            if self.listening_sound_file
            else None
        )
        ...
        self.command_ack_sound_path = (
            ...
            if self.command_ack_sound_file
            else None
        )
    ...
    def play_listening_sound(self) -> None:
        """Play a short backend cue when the assistant starts listening."""
        if not self.listening_sound_file:
            return
        if not self._backend_output_ready():
            return
        if not self.listening_sound_path:
            if not self.listening_sound_warning_shown:
                print(
                    f"Listening sound '{self.listening_sound_file}' not found. "
                    "Set LISTENING_SOUND_FILE to a WAV file or place it in assets/."
                )
                self.listening_sound_warning_shown = True
            return
        try:
            self.play_wav_file(self.listening_sound_path)
        except Exception as e:
            if not self.listening_sound_warning_shown:
    ...
    def play_command_ack_sound(self) -> None:
        ...
        if (
            not self.command_ack_sound_file
        ...
        def _play() -> None:
            try:
                if not self.command_ack_sound_path:
                    raise RuntimeError(f"{self.command_ack_sound_file} was not found in assets/")
