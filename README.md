...
The web UI shows three small sample indicators per profile; each square turns green after the corresponding WAV has been saved. When all three samples are present and Resemblyzer is installed, upload computes a normalized mean voice embedding and saves it as `profil1.npy` to `profil5.npy` next to the WAV files. Runtime loads that single mean embedding, so additional samples do not add per-command latency. Replacing any sample makes the mean embedding stale; the next complete upload recomputes it. Use short clean phrases from the person speaking normally. Avoid music, stage noise, reverb, long silences, clipping, heavy compression, and other voices. Mono WAV at 16 kHz or 44.1/48 kHz is fine; Resemblyzer will resample internally.
