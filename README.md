...
- `POST /api/web-tts`: returns browser-playable base64 MP3 speech when web audio TTS is enabled. The config page's voice test button can also pass temporary `provider`, `voice`, `model`, and `speed` values to preview the selected cloud voice without saving the profile first; browser playback volume is applied client-side during the preview.
