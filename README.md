...
When the web config requests a runtime reload while backend microphone capture is active, the assistant stops the recording loop and rebuilds itself. On reload only, PortAudio termination is deferred instead of calling `PyAudio.terminate()` immediately, and the local pyttsx3/espeak engine stop is skipped in favor of setting the shared TTS stop event. This avoids native ALSA/PipeWire crashes and pyttsx3/espeak hangs seen when tearing down audio directly after an interrupted capture.
