...
The Settings -> Config -> STT/TTS section exposes the most useful voice-activity settings as friendly sliders. Browser controls tune microphone sensitivity, minimum short-word duration, confirmation frames, end-of-phrase silence, and idle listener restart. Backend controls tune the equivalent PyAudio gate: sensitivity, minimum speech duration, confirmation frames, and end-of-phrase silence. Hover each slider label to see the exact `.env` variable it writes. A nested examples panel provides three starting points for quick isolated words, breath filtering, and slow soft speech; applying one fills the sliders and still requires Save to persist it.
