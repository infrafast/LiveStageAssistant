...
For HTTP/Streamable MCP servers, the web monitor Config tab shows a **MCP Servers** collapsible. Each HTTP server from the active `MCP_CONFIG` is listed with an **Open** link and a manual **Load frame** button pointing at that server's `/mcp` page, so compatible servers such as XMSeries-MCP or QLCPlus-MCP can expose their own runtime/admin UI beside the assistant configuration when they are reachable from the browser. Local stdio servers are listed as non-embeddable because they do not have a browser endpoint. If a server requires Bearer headers, the iframe cannot inject those headers; use the **Open** link or that server's own authentication flow.
