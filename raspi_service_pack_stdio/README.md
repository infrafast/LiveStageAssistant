...
For reliable recognition, upload three clean WAV samples per speaker slot from the web Config -> STT/TTS -> Voice Activity Detection -> Speaker profiles section. Profile slots use a fixed filename convention: profile 1 reads `profil1_1.wav`, `profil1_2.wav`, and `profil1_3.wav`; profile 2 reads `profil2_1.wav`, `profil2_2.wav`, and `profil2_3.wav`; and so on under `SPEAKER_PROFILES_DIR` (`data/speaker_profiles` locally, `/data/speaker_profiles` in Docker). Older single-file names such as `profil1.wav` are no longer used. When all three samples are present and Resemblyzer is installed, upload computes a normalized mean embedding and saves it as `profil1.npy` to `profil5.npy`, so runtime compares against one cached vector per profile and does not add per-command latency. Use short clean phrases from the same microphone path used live when possible; avoid music, crowd noise, reverb, clipping, long silences, and multiple voices.
