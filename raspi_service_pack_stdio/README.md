...
Set `COMMAND_ACK_SOUND_ENABLED=true` from the web Config -> Other section to play a short generated chime after the LLM/MCP response is ready and just before TTS generation/playback begins. The chime uses the selected speech side: backend PyAudio for backend TTS, or Web Audio for browser TTS. It does not stop the thinking sound.
