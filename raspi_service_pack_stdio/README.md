...
For reliable recognition, upload one clean WAV per speaker slot from the web Config -> STT/TTS -> Voice Activity Detection -> Speaker profiles section. Profile slots use a fixed filename convention: profile 1 reads `profil1.wav`, profile 2 reads `profil2.wav`, and so on under `SPEAKER_PROFILES_DIR` (`data/speaker_profiles` locally, `/data/speaker_profiles` in Docker). Uploads replace the corresponding `profilN.wav`. When Resemblyzer is installed, upload also saves `profil1.npy` to `profil5.npy` embeddings next to the WAV files, so runtime does not need to analyze the full reference sample again unless the WAV changes. Use 10 to 30 seconds of normal speech from the same microphone path used live when possible; avoid music, crowd noise, reverb, clipping, long silences, and multiple voices.
