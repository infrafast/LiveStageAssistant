...
Le web monitor expose maintenant un collapsible Settings > Config > MCP Servers. Les serveurs MCP HTTP du profil actif y sont listés avec Open via NAS, Direct et un chargement manuel de frame. Open via NAS et la frame passent par /api/mcp-admin/<server>/..., ce qui permet d'utiliser une page runtime/admin même si seul le backend LiveStageAssistant/NAS voit le serveur MCP via Tailscale. Les serveurs stdio restent visibles comme non embarquables; les headers Bearer configurés sont appliqués côté backend et ne sont pas exposés au navigateur.
