# LiveStageAssistant Architecture

This document collects the technical architecture, runtime behavior, MCP integration details, and future RAG direction for Live Stage Assistant.

The user-facing installation and usage guide lives in [README.md](../README.md). Deployment-specific guides remain in [raspi_service_pack_stdio/README.md](../raspi_service_pack_stdio/README.md) and [docs/synology-docker.md](synology-docker.md).
...
## Docker/Synology Architecture Notes

The Docker image is straightforward from an application point of view: it packages a Python service with Node.js available for MCP servers. The important design constraints are runtime integrations:

- Microphone and speaker access require `/dev/snd` passthrough and working ALSA support on the NAS or host.
- The mixer and lighting MCP servers need network reachability from the assistant container. Bridge networking works when HTTP MCP endpoints use a reachable LAN, Docker service, or Tailscale address instead of container-local `127.0.0.1`.
- Offline mode needs Ollama and local Whisper model caches available to the container or reachable from it.
- Local stdio MCP servers such as XMSeries-MCP or QLCPlus-MCP must be mounted after they have been installed and built.

For a first Synology deployment, prefer the web monitor and text command injection with browser or silent TTS. This validates the assistant, MCP connectivity, API keys, and web monitor before adding backend audio passthrough, which is host-specific and more fragile.

The practical Synology deployment procedure lives in [docs/synology-docker.md](synology-docker.md).

## Detailed Runtime Reference

The following sections describe the lower-level runtime behavior that used to live in the main README. Keep detailed implementation explanations here so the README can stay focused on installation and normal use.
...
VAD_MIN_SPEECH_MS=250                            # Minimum speech duration before STT is accepted
...
BACKEND_WAKE_WORD_MODEL_PATHS=                   # Comma-separated openWakeWord model files, required when WAKE_WORD is set
BACKEND_WAKE_WORD_MODEL_NAMES=                   # Comma-separated openWakeWord bundled/custom model names
BACKEND_WAKE_WORD_THRESHOLD=0.65                 # Streaming wake score required to activate
...
LISTENING_SOUND_FILE=                           # Optional backend WAV cue before direct no-wake listening; empty disables it
WAKE_DETECTED_SOUND_FILE=                       # Optional backend WAV cue after openWakeWord triggers; empty disables it
...
COMMAND_ACK_SOUND_FILE=                       # Optional WAV cue when the response is ready and TTS is about to start
BACKEND_AUDIO_INPUT_DEVICE=                     # Optional PyAudio input index or pipewire:source:<node.name>; empty uses default
BACKEND_AUDIO_INPUT_GAIN=1.00                    # Backend microphone software gain, 0.50 to 2.00
BACKEND_AUDIO_OUTPUT_DEVICE=                    # Optional PyAudio output index or pipewire:sink:<node.name>; empty uses default
...
Backend microphone wake gating is now strict. `WAKE_WORD=` disables wake-word detection and lets Silero VAD endpoint backend commands directly. When `WAKE_WORD` contains one or more wake words, the backend requires openWakeWord plus at least one model in `BACKEND_WAKE_WORD_MODEL_PATHS` or `BACKEND_WAKE_WORD_MODEL_NAMES`; there is no silent fallback to a post-transcription text gate. On Linux/Raspberry Pi, `scripts/install.sh` installs openWakeWord in ONNX-only mode, downloads the internal ONNX resources such as `melspectrogram.onnx` and `embedding_model.onnx`, and validates the setup by instantiating an ONNX `Model`.

With wake word enabled, the backend microphone is read inside the normal listening loop and the latest `BACKEND_WAKE_WORD_PRE_ROLL_MS` of audio is retained only until openWakeWord decides that the wake word fired. At that point pre-trigger audio is discarded and a fresh Silero VAD segment starts immediately for the command. The wake decision is not revalidated by Whisper; after STT the assistant only strips a leading wake word from the transcription when present. After backend TTS finishes, the microphone continues to drain normally but openWakeWord/VAD input is suppressed for 350 ms and their state is reset, preventing the assistant's own acoustic/loopback tail from retriggering the wake word without adding a blocking sleep. If the wake detector fires on silence or noise and no command speech follows, the assistant returns to listening without calling STT. If openWakeWord cannot be imported, has no configured model, has missing ONNX resources, or fails during capture, backend microphone capture pauses and the log reports the reason. Browser conversation audio, browser push-to-talk, and WAV files uploaded through the chat composer keep their own web-path handling for now and must not reintroduce a backend `generic/post_stt` mode.

The backend audio loop now exposes explicit runtime states: `WAIT_WAKE`, `CAPTURE_COMMAND`, `PROCESSING`, and `TTS`. With a configured wake word, the loop starts in `WAIT_WAKE`; the microphone feeds openWakeWord only, so Silero VAD does not endpoint ambient speech before authorization. Once openWakeWord triggers, the state changes to `CAPTURE_COMMAND` and Silero V6 owns command endpointing. When command audio is captured, the state changes to `PROCESSING` for STT, speaker recognition, LLM, and MCP work, then to `TTS` while the response is spoken. With `WAKE_WORD=`, the loop starts directly in `CAPTURE_COMMAND`, then follows `PROCESSING -> TTS -> CAPTURE_COMMAND`. For accepted backend audio, accepted browser audio uploads, and optional voice-interruption commands, STT and speaker recognition run in parallel when speaker recognition is enabled and available; if it is disabled or unavailable, the assistant runs only STT and keeps the former latency profile.
...
- `POST /api/speaker-profile-sample`: previews a validated speaker-profile WAV through the TTS output currently selected in the web form; browser playback returns base64 WAV audio, while backend playback uses the requested device, volume, and pan.
...
Backend microphone STT and browser microphone STT now use the same bundled Silero VAD ONNX model offline. Backend microphone input uses `STT_PROVIDER`, `LOCAL_WHISPER_MODEL`, `STT_LANGUAGE`, and `STT_PROMPT` for transcription selection and Whisper biasing; browser microphone input uses `WEB_STT_PROVIDER` and `WEB_STT_MODEL`. `STT_LANGUAGE` is also the web GUI locale and is selected from Settings -> Config -> **User interface**. Available choices are discovered from `assets/i18n/<locale>.json`; the repository ships `fr` and `en`, and changing the language saves the active `.env` file and reloads the assistant. Before browser audio or no-wake backend audio sends audio to STT, Silero estimates speech probability. With `WAKE_WORD` configured, the backend microphone first waits for local openWakeWord activation, keeps `BACKEND_WAKE_WORD_PRE_ROLL_MS` of pre-roll audio, then requires at least `VAD_MIN_SPEECH_MS` of command speech before STT and uses Silero to decide when the accepted command ends. `VAD_SPEECH_THRESHOLD` starts speech, `VAD_NEGATIVE_THRESHOLD` allows accepted speech to end, `VAD_MIN_SPEECH_MS` rejects tiny noises, `VAD_MIN_SILENCE_MS` controls end-of-phrase timing, `VAD_SPEECH_PAD_MS` keeps backend pre-roll before detected speech when wake word is disabled, and `VAD_MAX_SPEECH_SECONDS` caps one utterance. Raise thresholds or minimum speech values to reject breaths/noise; lower them if short spoken commands are missed.
...
Cloud API errors are normalized before they reach the user. OpenAI and ElevenLabs quota, credit, authentication, and temporary rate-limit failures are classified into short French messages instead of exposing raw provider exceptions. If backend STT fails because cloud credit is exhausted, the voice loop logs the cause, shows a web dialogue message, plays `assets/insuficientAPIcredit.wav` through the configured backend output, then returns to listening without sending anything to the LLM or MCP servers. If that WAV is missing or cannot be played, the assistant falls back to local `pyttsx3` on the configured backend output. LLM quota errors return a concise assistant response, and cloud TTS quota errors use the same local alert path so the warning does not depend on the unavailable cloud TTS provider.
...
`INTERRUPT_CONVERSATION_ENABLED=false` keeps the conservative default: text, web STT, and backend STT do not start a new normal command while the assistant is processing. The web GUI exposes the same setting under **STT/TTS -> Interrupt conversation** and saves it to the active `.env` profile. When set to `true`, a new text command or accepted STT command silently cancels current processing or TTS first, then runs the new command. Browser conversation mode also keeps listening during processing and web TTS.

Backend voice interruption uses the same `record_audio()` state-machine path as normal backend listening. It no longer has a special short microphone capture path. If `WAKE_WORD` is configured, interruption starts in the existing activity state and only moves to `CAPTURE_COMMAND` after openWakeWord detects the wake word; Silero then endpoints the replacement command. If `WAKE_WORD=` and interruption is enabled, Silero may detect a new spoken command during `PROCESSING` or `TTS`, which is naturally more sensitive to stage noise. A spoken cancel phrase such as `stop` or `annule` cancels without queueing a new command. Backend barge-in still depends on microphone availability and can be affected by the assistant's own speaker output leaking into the mic; for the cleanest backend interruption, use headphones, echo control, or browser TTS instead of open speakers.

Backend audio devices can be selected from Settings -> Config -> Audio In/Out. Backend input values can be PyAudio device indexes or explicit PipeWire sources saved as `BACKEND_AUDIO_INPUT_DEVICE=pipewire:source:<node.name>`; leave the value empty to use the system default. Backend output values can be either PyAudio device indexes or explicit PipeWire sinks saved as `BACKEND_AUDIO_OUTPUT_DEVICE=pipewire:sink:<node.name>`. PipeWire nodes are discovered with `pw-dump`; backend output uses `pw-play --target`, and backend input uses `pw-cat`/`pw-record --target`, so a Raspberry Pi can route LiveStageAssistant to one sound card without changing the global default source or sink used by QLC+, MPD, or other apps. Browser input/output device choices remain browser-local and are not stored in `.env`.

`BACKEND_AUDIO_INPUT_GAIN` applies a saturating `0.50` to `2.00` software gain immediately after microphone capture, before VAD, STT, diagnostics, monitoring, and speaker-profile recording. The diagnostic test uses the unsaved GUI value immediately and its replay WAV contains the gained signal. If a saved backend input or output is invalid or unavailable at startup, the backend marks that side unavailable instead of silently falling back to another system device; the web monitor remains available so the selection can be fixed. If the web config is saved while a previously selected backend device is unavailable, the stale selection is cleared to the default instead of blocking the save. The older separate `Audio input` state tile is replaced by this single backend audio tile. Backend microphone recording uses the selected input device; PyAudio/ALSA inputs are opened with a supported channel/rate combination when ALSA rejects the 16 kHz default, while PipeWire inputs are captured as targeted 16 kHz mono PCM. Audio is resampled internally to 16 kHz for Silero VAD when needed. Backend cloud TTS and backend thinking sound play through the selected backend output only when TTS Output is `Backend`; when TTS Output is `Browser`, the browser plays web TTS and the thinking sound through its own selected/browser-default output. MP3 cloud TTS is decoded with `ffmpeg` before playback and never delegated to an external default-device player. The backend thinking sound continues through cloud/local TTS generation and stops only at the transition into actual TTS playback, avoiding a silent gap between the answer being ready and speech starting. The **User interface** panel lists every top-level `assets/*.wav` file for thinking sound, backend listening cue, wake-detected cue, startup loader, and command acknowledgement sound. Their Play buttons use the currently selected output and volume; thinking-sound and command-acknowledgement preview support Browser and Backend, the listening and wake-detected cues always preview through the selected backend output, and loader selection/preview are intentionally disabled unless TTS Output is Backend. `LISTENING_SOUND_FILE` is played only when wake word is disabled, after the backend microphone stream has opened and before direct command capture begins. `WAKE_DETECTED_SOUND_FILE` is played asynchronously when openWakeWord triggers during normal backend listening; capture continues while it plays, so `momo monte Claude` remains possible. Selecting the disabled entry stores an empty value. Selecting **No startup loader sound** saves `STARTUP_LOADER_SOUND_ENABLED=false`; selecting a WAV saves it as `STARTUP_LOADER_SOUND_FILE` and enables the loader. At runtime the loader remains backend-only, loops while the assistant initializes, and stops just before the startup TTS says `Assistant vocal prêt`. `COMMAND_ACK_SOUND_FILE` selects the WAV played after the LLM/MCP response is ready and just before TTS generation/playback begins; an empty value disables it. It follows the selected speech side, using backend output for backend TTS and browser audio for browser TTS, and does not stop the thinking sound. Local `pyttsx3` is first rendered to a file and played through the same controlled backend output path; direct system TTS fallback is disabled so speech cannot escape to the OS default device. `BACKEND_TTS_VOLUME` applies software gain to backend TTS, backend thinking-sound playback, backend listening-cue playback, backend wake-detected playback, backend command-acknowledgement playback, and backend startup-loader playback before PyAudio or PipeWire playback; `BACKEND_AUDIO_OUTPUT_PAN` applies software pan from left `-1.00` to center `0.00` to right `1.00` on the same backend playback path. `BACKEND_AUDIO_MONITOR_MODE=passthrough` forwards backend microphone chunks to the backend output while capture is running for PyAudio outputs; with a PipeWire sink selected, pass-through monitoring is disabled while `rejected` replay still uses the selected PipeWire sink. `BACKEND_AUDIO_MONITOR_MODE=rejected` replays a captured phrase only when it is rejected because the wake word was not detected, and is therefore available only when `WAKE_WORD` is set. `BACKEND_AUDIO_MONITOR_VOLUME` controls that microphone monitoring path separately from TTS gain.

Speaker-profile sample previews follow the TTS Output currently selected in the form, without requiring Save first. Browser uses the selected browser output and browser TTS volume; Backend uses the selected backend output, backend TTS volume, and pan; Silent disables the sample Play buttons with an explanatory tooltip.

The sound-preview Play buttons are toggles: while active they loop the selected WAV and display a Stop icon; pressing them again stops playback immediately on both Browser and Backend output.

Startup logs include elapsed timings for Silero/ONNX initialization, speaker-recognition validation, PyAudio creation, backend device resolution, backend input-format probing, and the complete `VoiceAssistant` constructor. These lines help identify slow imports or ALSA format probes on Raspberry Pi without changing startup behavior.
...
Browser/web TTS never falls back to pyttsx3. If the selected web cloud provider cannot be used, browser audio falls back to silent text chat and the monitor shows a short red status message, such as exhausted credits, API-key refusal, or rate limit, instead of the raw provider error. Backend/non-web cloud TTS is decoded and played only through the configured backend output; direct OS-default playback is disabled.

Because the web UI polls snapshots, it keeps only the latest observed assistant-bubble key (`id` plus `created_at`) so the same bubble is spoken once rather than after every poll. The timestamp keeps newly created responses distinct when numeric IDs restart after a reload, while restored history remains silent.
...
During those startup probes, stdio MCP server stderr is captured silently so the console does not show duplicate server startup logs. If a probe fails, the assistant includes the captured stderr tail in the skipped-server warning. The real MCP sessions still show their normal server logs.
...
The network status announcement uses the newly selected profile. Backend TTS uses the selected backend provider. Browser TTS profiles are also handled server-side for this status announcement when `TTS_PROVIDER=none` and `WEB_TTS_PROVIDER=openai|elevenlabs`; the web monitor also marks the status message for browser playback when the UI is open. If the selected cloud provider or configured backend output cannot be used, backend announcements are skipped instead of falling back to the OS default audio route.
...
For short stage commands, `STT_PROMPT` can give Whisper mixer-specific context. It must not contain the active wake word: wake detection is handled by `WAKE_WORD` and, optionally, openWakeWord before STT. The bundled default biases French mixer commands such as `mets Claude`, `baisse snare`, `mute Voc-Claude`, and the assistant also fixes the narrow transcription artifact where a leading `mets` command is fused with the following channel name. At runtime, any `assistantOptions.routing` keywords from the active `MCP_CONFIG` are appended to the STT prompt without rewriting the env file. The web config exposes the base value under **Config -> STT/TTS**.
...
Pass different parameters when initializing. For offline mode, use `tts_provider="pyttsx3"`; for silent text-only output, use `tts_provider="none"`. For backend/non-web cloud speech, use `tts_provider="openai"` or `tts_provider="elevenlabs"`; those cloud modes use only the configured backend output route.
...
`SPEAKER_RECOGNITION_TIMEOUT_SECONDS` bounds each speaker analysis. STT and speaker recognition are launched together for an accepted utterance, so the LLM waits only for the slower of the two bounded stages instead of paying both durations sequentially. If speaker recognition expires, the accepted transcription continues with speaker `unknown`, speaker recognition is disabled for the remainder of that assistant session, and runtime state reports the timeout. This prevents optional voice identification from blocking command handling indefinitely.

With `DEBUG=true`, backend audio adds structured timing lines without changing runtime behavior. `Backend audio timing:` reports wake detection delay, command speech start, command audio duration, endpoint silence, and total capture time. `Audio analysis timing:` reports STT, speaker recognition when active, and combined pre-LLM duration. During `WAIT_WAKE`, openWakeWord also emits throttled debug lines with current score, recent max score, threshold, rejected reason, and immediate trigger details; normal logs stay quiet.
...
2. **TTS Not Working**
   ...
   - Backend cloud TTS is played only through the configured backend output; if that route fails, speech is skipped so it cannot leak to another device
   - Browser/web TTS does not fall back to pyttsx3; set `TTS_PROVIDER=none` and `WEB_TTS_PROVIDER=openai` or `elevenlabs`
   - On Ubuntu/Debian/Raspberry Pi OS, install the system TTS/audio packages: `sudo apt-get install alsa-utils ffmpeg pipewire-bin espeak espeak-ng libespeak1 libespeak-ng1`
   - In headless environments without ALSA/PyAudio output, `ffmpeg`, or `aplay`, spoken output is skipped without noisy playback errors. Use `TTS_PROVIDER=none` to make silent mode explicit.
...
4. **Thinking Sound Or Audio Output Unavailable**
   - If PyAudio cannot open the selected backend output device, choose a working backend output in the web config; leave `BACKEND_AUDIO_OUTPUT_DEVICE` empty only when you intentionally want the system default
