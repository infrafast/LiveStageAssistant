...
Backend microphone wake gating has two modes. `BACKEND_WAKE_WORD_MODE=post_stt` keeps the legacy path: Silero VAD records a phrase, STT transcribes it, and `WAKE_WORD` is matched against the resulting text. `BACKEND_WAKE_WORD_MODE=openwakeword` enables an optional local streaming gate when the `openwakeword` package and at least one model in `BACKEND_WAKE_WORD_MODEL_PATHS` or `BACKEND_WAKE_WORD_MODEL_NAMES` are available. On Linux/Raspberry Pi, `scripts/install.sh` installs openWakeWord in ONNX-only mode, downloads the internal ONNX resources such as `melspectrogram.onnx` and `embedding_model.onnx`, and validates the setup by instantiating an ONNX `Model`. In streaming mode, the backend microphone is read continuously inside the normal listening loop and the latest `BACKEND_WAKE_WORD_PRE_ROLL_MS` of audio is retained only until openWakeWord decides that the wake word fired. At that point all pre-trigger audio is discarded and a fresh VAD segment starts immediately for the command, so the wake phrase itself cannot reach STT or the LLM. After backend TTS finishes, the microphone continues to drain normally but openWakeWord/VAD input is suppressed for 350 ms and their state is reset, preventing the assistant's own acoustic/loopback tail from retriggering the wake word without adding a blocking sleep. If the wake detector fires on silence or noise and no command speech follows, the assistant returns to listening without calling STT. No extra post-trigger delay is introduced; command capture continues on the next audio chunk.

Streaming wake detection is backend-only for now. Browser conversation audio, browser push-to-talk, and WAV files uploaded through the chat composer keep the post-STT wake-word gate so the web path remains stable. If openWakeWord cannot be imported, has no configured model, has missing ONNX resources, or fails during capture, the assistant logs the reason and falls back to the generic post-STT gate instead of disabling the web monitor or backend microphone. When `BACKEND_AUDIO_MONITOR_MODE=rejected` is enabled with openWakeWord, backend VAD also buffers speech heard before any wake detection; if that phrase ends without the wake word firing, the rejected phrase is replayed through the configured backend output just like the generic post-STT gate. The backend listen log is always explicit: `Listening (no wake word)`, `Listening for "momo" using openwakeword...`, or `Listening for "régie, console" using generic...`.
