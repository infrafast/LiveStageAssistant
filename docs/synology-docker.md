...
When `COMMAND_ACK_SOUND_ENABLED=true`, the assistant also plays a short generated chime as soon as a command is accepted for LLM/MCP processing; it uses backend PyAudio for backend TTS and Web Audio for browser TTS, and it does not stop the thinking sound.
