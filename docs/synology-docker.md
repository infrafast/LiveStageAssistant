...
Backend wake-word detection can optionally use a local streaming openWakeWord model with `BACKEND_WAKE_WORD_MODE=openwakeword` plus `BACKEND_WAKE_WORD_MODEL_PATHS` or `BACKEND_WAKE_WORD_MODEL_NAMES`. On Linux/Raspberry Pi, `scripts/install.sh` installs openWakeWord in ONNX-only mode, downloads its required internal ONNX resources, and validates the install by instantiating an ONNX model. In streaming mode, only the backend microphone path changes: the assistant keeps a short backend audio ring buffer, waits for the local wake detector, and starts STT after activation so the beginning of the phrase is not cut off. Browser STT and uploaded WAV commands continue to use the post-STT wake-word gate. Backend listen logs use one explicit format, for example `Listening for "momo" using openwakeword...` or `Listening for "régie, console" using generic...`.
