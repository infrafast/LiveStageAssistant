...
When `COMMAND_ACK_SOUND_FILE` names an `assets/*.wav` file, the assistant also plays that WAV after the LLM/MCP response is ready and just before TTS generation/playback begins; it uses backend PyAudio for backend TTS and browser audio for browser TTS, and it does not stop the thinking sound. Empty `COMMAND_ACK_SOUND_FILE` disables this cue.
